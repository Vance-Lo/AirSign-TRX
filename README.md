...
* **🧩 SSS 分片技术**: 私钥生成后即被拆分为 5 份（3/5 门限），任何单一物理介质丢失都不会导致资产失窃 [cite: 2026-01-17]。
* **🛡️ 字节级签名**: 采用 SHA256 原始字节提取算法，解决 Protobuf 跨平台序列化哈希不一致问题，杜绝盲签风险 [cite: 2026-02-17]。
* **👁️ 智能反欺诈**: 冷端在签名时自动解析交易内容，直观显示收款地址与金额 [cite: 2026-02-17]。
* **💵 全币种支持**: 原生支持 TRX 转账及 USDT (TRC20) 合约调用 [cite: 2026-01-17]。
...
你需要安装 Go 1.16+ 环境 [cite: 2026-01-16]。
...
* **安全**: 完成后建议重启断网电脑清空内存 [cite: 2026-02-17]。
...
### 5. 规范“免责声明”的视觉效果
**修改点**：使用引用块 `>` 增加法律条款的严肃感。
**请替换为：**
```markdown
## ⚠️ 免责声明 (Disclaimer)

> 本软件按“原样”提供，不提供任何形式的明示或暗示保证 [cite: 2026-02-17]。作者不对因使用本软件而导致的资金丢失、私钥泄露或交易失败承担任何责任 [cite: 2026-02-17]。

* 请务必在小额测试通过后再进行大额资产管理 [cite: 2026-02-17]。
* 请务必保管好您的物理分片文件 [cite: 2026-01-17]。
* **严禁在联网设备上运行 Vault 程序** [cite: 2026-01-17]。
